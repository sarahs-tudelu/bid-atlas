...
The active catalog admits a project when its canopy/pergola/partition-wall product-fit score is at least 8. Projects without a usable source-published email address or phone number remain visible and are labeled `Research needed`. Phone-capable projects expose a user-initiated call action, while email outreach remains restricted to source-published email addresses. A verified `@tudelu.com` Google login owns the workspace. Reviewed email defaults to the configured marketing mailbox, can select another provider-authorized marketing account with a designated Tudelu sales reply owner, or can explicitly use the employee's own Gmail account. Full inbox bodies are not retained, and a successful selected-provider response is required before sent status is recorded. See [`COLD_OUTREACH_INTEGRATION.md`](COLD_OUTREACH_INTEGRATION.md).
...
Source adapters that explicitly report `snapshotComplete: false` merge their bounded page into previously loaded rows instead of deleting unseen projects. The API read model then merges only conservative cross-database duplicate matches and retains every contributing official ID and URL. Raw source counts remain separate from merged project counts.
